...
class CourseChapterItemFrag : Fragment(), View.OnClickListener, PlayerHelper.Listener,
    ...
    private fun initSessionItem() {
        ...
        if (mSessionItem!!.date * 1000 <= System.currentTimeMillis()) {
            ...
            mBinding.courseChapterItemBtnContainerTv.text =
                ("${getString(R.string.session_conducted)} ${Utils.getDateTimeFromTimestamp(mSessionItem!!.date)}")
        } else {
            status = getString(R.string.not_conducted)
            mBinding.courseChapterItemBtnContainerTv.text =
                ("${getString(R.string.session_start_at)} ${Utils.getDateTimeFromTimestamp(mSessionItem!!.date)}")
        }
    ...
    private fun initFileItem() {
        if (mFileItem!!.downloadable.toBoolean() && mFileItem!!.storage == ChapterFileItem.Storage.LOCAL.value && !mOfflineMode) {
        ...
        if ((mFileItem!!.storage == ChapterFileItem.Storage.LOCAL.value && mFileItem!!.fileType.toUpperCase(
                Locale.ENGLISH
            ) in Utils.VIDEO_FORMATS)
        ) {
        ...
        } else if ((mFileItem!!.storage == ChapterFileItem.Storage.ONLINE.value && mFileItem!!.fileType.toUpperCase(
                Locale.ENGLISH
    ...
    private fun getVideoFromLocalDir(): File? {
        if (mFileItem!!.storage == ChapterFileItem.Storage.LOCAL.value) {
    ...
    private fun initTextItem() {
        ...
        val green = ContextCompat.getColor(requireContext(), R.color.green_accent)
        ...
        mBinding.courseChapterItemEndBtn.backgroundTintList =
            ContextCompat.getColorStateList(requireContext(), R.color.green_accent)
    ...
    private fun initTTS() {
        ...
        if (mTextToSpeech == null) {
            mTextToSpeech = TextToSpeech(requireContext()
            ) { status ->
